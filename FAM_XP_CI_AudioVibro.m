...
try
    ...
    experiment = 'Familiarization-XP-CI-AudioVibroRhythm'; % name of the experiment
    if modality == 1
        session = 'VIBRO';
    else
        session = 'AUDIO';
    ...
    logfileName = sprintf('log_%s_%s_%s_sub%s_%s.mat',experiment,session,subID,timestamp);
    
    logtxtName = sprintf('log_%s_sub%s_bloc%s_%s_%s.txt',experiment,subID,session,timestamp); % create text file for this subject and day-time
    ...
    trials=[1 1 2 2]; % two first are standard and 2 last deviant trials
...
%             end
        ...
        if trialTerminated
            fileNameTap = fullfile(logPath, sprintf('%s_%s_ID%s_%s_%s_%s_event%d_trial%d_TERMINATED.wav', experiment, session, subID, timestamp, trackName, task, eventi, triali));
        else
            fileNameTap = fullfile(logPath, sprintf('%s_%s_ID%s_%s_%s_%s_event%d_trial%d.wav', experiment, session, subID, timestamp, trackName, task, eventi, triali));
        end
        disp('saving data...')
